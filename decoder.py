...
class Encoder(torch.nn.Module):
    def __init__(self,n_inputs, n_outputs, factor=6, bn='before'):
        ...
        n_hidden = factor*128
        if bn == 'before':
            self.net = torch.nn.Sequential(torch.nn.Linear(n_inputs, n_hidden),
...
def train_model(coordinates_from, coordinates_to, file_name, method_type,
                n_epochs=1000, space='poincare', lr=1e-4, wd=1e-3, batch_size=8,
                n_warmup=3000, cuda=0, tb=0, bn='before', lrm=1.0):
    ...
    encoder = Encoder(n_inputs=coordinates_from.shape[1],
                      n_outputs=coordinates_to.shape[1], bn=bn)
...
if __name__ == "__main__":  
    ...
    data, true_labels, col_names = read_data(f"datasets/{model_name}",
    ...
    if opt.method == 'poincare':
        ...
        plot_embedding(poincare_coord, labels=true_labels, coldict=color_dict,
